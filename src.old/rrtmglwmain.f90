...
! --------- Modules ----------
      ...
      !USE MOD_AEROSOLS, Only: macnbands
! ------- Declarations -------
...
! by scaling mass mixing ratio (g/g) with the appropriate molecular weights (g/mol) 
       integer(kind=im), intent(in) :: KST, KEND
      ...
      real(kind=rb), intent(in) :: play(KST:KEND,nlay) ! Layer pressures (hPa, mb)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: plev(KST:KEND,nlay+1)  ! Interface pressures (hPa, mb)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=rb), intent(in) :: tlay(KST:KEND,nlay)          ! Layer temperatures (K)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: tlev(KST:KEND,nlay+1)          ! Interface temperatures (K)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=rb), intent(in) :: tsfc(KST:KEND)            ! Surface temperature (K)
                                                      !    Dimensions: (ncol)
      real(kind=rb), intent(in) :: h2ovmr(KST:KEND,nlay)        ! H2O volume mixing ratio
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: o3vmr(KST:KEND,nlay)         ! O3 volume mixing ratio
                                                      !    Dimensions: (ncol,nlay)
...
!                                                      !    Dimensions: (ncol,nbndlw)
     real(kind=rb), intent(in) :: emis(KST:KEND)          ! Surface emissivity
                                                      !    Dimensions: (ncol)
     real(kind=rb), intent(in) :: emiw(KST:KEND)          ! Surface window emissivity
                                                      !    Dimensions: (ncol)
      ...
      real(kind=rb), intent(in) :: cldfr(KST:KEND,nlay)         ! Cloud fraction
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: cicewp(KST:KEND,nlay)        ! Cloud ice water path (g/m2)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: cliqwp(KST:KEND,nlay)        ! Cloud liquid water path (g/m2)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: reice(KST:KEND,nlay)         ! Cloud ice particle effective size (microns)
                                                      !    Dimensions: (ncol,nlay)
                                                      ...
                                                      !               [dge = 1.0315 * r_ec]
      real(kind=rb), intent(in) :: reliq(KST:KEND,nlay)         ! Cloud water drop effective radius (microns)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: taucld(KST:KEND,nlay,nbndlw)      ! In-cloud optical depth
...
!      real(kind=rb), intent(in) :: asmcld(:,:,:)     ! In-cloud asymmetry parameter
                                                      ...
                                                      !   for future expansion 
                                                      !   (lw scattering not yet available)
      real(kind=rb), intent(in), Optional :: tauaer(KST:KEND,5,nlay)      ! aerosol optical depth
...
!      real(kind=rb), intent(in) :: asmaer(:,:,:)     ! aerosol asymmetry parameter
      ...
      !real(kind=rb), intent(in), Optional :: tauaer_MacV2(KST:KEND,macnbands,nlay)      ! aerosol optical depth


! ----- Output -----

      real(kind=8), intent(out) :: uflx(KST:KEND,nlay+1)         ! Total sky longwave upward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=8), intent(out) :: dflx(KST:KEND,nlay+1)         ! Total sky longwave downward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
!      real(kind=rb), intent(out) :: hr(KST:KEND,nlay)           ! Total sky longwave radiative heating rate (K/d)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=4), intent(out) :: uflxc(KST:KEND,nlay+1)        ! Clear sky longwave upward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=4), intent(out) :: dflxc(KST:KEND,nlay+1)        ! Clear sky longwave downward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
!      real(kind=rb), intent(out) :: hrc(KST:KEND,nlay)          ! Clear sky longwave radiative heating rate (K/d)
                                                      !    Dimensions: (ncol,nlay)
...
! Atmosphere/clouds - cldprop
      ...
      real(kind=rb) :: cldfrac(0:nlay+1)        ! layer cloud fraction
...
!              cfc22vmr, ccl4vmr,
               emis, emiw,inflglw, iceflglw, liqflglw, &
              cldfr, taucld, cicewp, cliqwp, reice, reliq, tauaer, & !tauaer_MacV2,  &
...
!        if (myid > 50) then 
!              write(*,*) myid,RLWUH
!              write(*,*)myid, minval(taucloud),maxval(taucloud)
!        endif
        if (inflag /=0) taucloud=max(1.e-14_rb,taucloud)*RLWUH
...
!***************************************************************************
      subroutine inatm (KST,KEND,iplon, nlay, icld, iaer, &
              play, plev, tlay, tlev, tsfc, Ph2o, &
              Po3, Pco2, & !  ch4vmr, n2ovmr, o2vmr, cfc11vmr, cfc12vmr, &
!              cfc22vmr, ccl4vmr, 
              emis, emiw, inflglw, iceflglw, liqflglw, &
              cldfr, taucld, cicewp, cliqwp, reice, reliq, tauaer, & !tauaer_MacV2, &
              nlayers, pavel, pz, tavel, tz, tbound, semiss, coldry, &
...
! --------- Modules ----------
      ...
      USE YOMPHY3, ONLY :  RCH4     ,RN2O    ,RCFC11  ,RCFC12,RO2
      !USE MOD_AEROSOLS, Only: LAER_MACv2, macnbands
! ------- Declarations -------
...
! by scaling mass mixing ratio (g/g) with the appropriate molecular weights (g/mol) 
      ...
      real(kind=rb), intent(in) :: tsfc(KST:KEND)            ! Surface temperature (K)
                                                      !    Dimensions: (ncol)
      real(kind=rb), intent(in) :: Ph2o(KST:KEND,nlay)        ! H2O volume mixing ratio
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: Po3(KST:KEND,nlay)         ! O3 volume mixing ratio
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: Pco2
!      real(kind=rb), intent(in) :: co2vmr(:,:)        ! CO2 volume mixing ratio
!                                                      !    Dimensions: (ncol,nlay)
!      real(kind=rb), intent(in) :: ch4vmr(:,:)        ! Methane volume mixing ratio
!                                                      !    Dimensions: (ncol,nlay)
...
!                                                      !    Dimensions: (ncol,nbndlw)
     real(kind=rb), intent(in) :: emis(KST:KEND)          ! Surface emissivity
                                                      !    Dimensions: (ncol)
     real(kind=rb), intent(in) :: emiw(KST:KEND)          ! Surface window emissivity (bands 6,7,8)
                                                      !    Dimensions: (ncol)
      ...
      real(kind=rb), intent(in) :: cliqwp(KST:KEND,nlay)        ! Cloud liquid water path (g/m2)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: reice(KST:KEND,nlay)         ! Cloud ice effective size (microns)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: reliq(KST:KEND,nlay)         ! Cloud water drop effective radius (microns)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=rb), intent(in) :: taucld(KST:KEND,nlay,nbndlw)      ! In-cloud optical depth
...
! Atmosphere
      ...
      real(kind=rb), intent(out) :: pavel(nlay)          ! layer pressures (mb) 
                                                      !    Dimensions: (nlay)
      real(kind=rb), intent(out) :: tavel(nlay)          ! layer temperatures (K)
                                                      !    Dimensions: (nlay)
...
! Atmosphere/clouds - cldprop
      ...
      real(kind=rb), intent(out) :: cldfrac(0:nlay+1)        ! layer cloud fraction
                                                      !    Dimensions: (nlay)
      real(kind=rb), intent(inout) :: ciwp(nlay+1)           ! cloud ice water path
                                                      !    Dimensions: (nlay)
      real(kind=rb), intent(inout) :: clwp(nlay+1)           ! cloud liquid water path
                                                      !    Dimensions: (nlay)
      ...
      real(kind=rb), intent(out) :: rei(:)            ! cloud ice particle effective size (microns)
                                                      !    Dimensions: (nlay)
      real(kind=rb), intent(out) :: tauc(nbndlw,nlay+1)         ! in-cloud optical depth
...
!  from GCM fields should be reversed here if necessary.
      ...
      do l = 1, nlayers
         pavel(l) = play(iplon,nlay-l+1)/100._8
         tavel(l) = tlay(iplon,nlay-l+1)
         pz(l) = plev(iplon,nlay-l+1)/100._8
         tz(l) = tlev(iplon,nlay-l+1)
! For h2o input in vmr:
!         wkl(1,l) = h2ovmr(iplon,l)
! For h2o input in mmr:
         wkl(1,l) = Ph2o(iplon,nlay-l+1)*amdw
! For h2o input in specific humidity;
!         wkl(1,l) = (Ph2o(iplon,nlay-l+1)/(1._rb - Ph2o(iplon,nlay-l+1)))*amdw
         wkl(2,l) = Pco2*amdc !co2vmr(iplon,l)
         wkl(3,l) = Po3(iplon,nlay-l+1)*amdo!o3vmr(iplon,l)
         wkl(4,l) = rn2o*amdn !vmr(iplon,l)
         wkl(6,l) = rch4*amdm !ch4vmr(iplon,l)
         wkl(7,l) = ro2*amdo2 !o2vmr(iplon,l)
...
! Set cross section molecule amounts from input; convert to vmr if necessary
      do l=1, nlayers
         wx(1,l) = 0. !*amd/amcl4!ccl4vmr(iplon,l)
...
!             if (LAER_MACv2) then
!               taua(l, 1) = tauaer_MacV2(iplon, 1,nlay+1-l)
!               taua(l, 2) = tauaer_MacV2(iplon, 3,nlay+1-l)
!               taua(l, 3) = tauaer_MacV2(iplon, 4,nlay+1-l)                        
!               taua(l, 4) = tauaer_MacV2(iplon, 5,nlay+1-l)
!               taua(l, 5) = tauaer_MacV2(iplon, 5,nlay+1-l)
!               taua(l, 6) = tauaer_MacV2(iplon, 6,nlay+1-l)
!               taua(l, 7) = tauaer_MacV2(iplon, 6,nlay+1-l)
!               taua(l, 8) = tauaer_MacV2(iplon, 7,nlay+1-l)  
!               taua(l, 9) = tauaer_MacV2(iplon, 7,nlay+1-l)
!               taua(l,10) = tauaer_MacV2(iplon, 8,nlay+1-l)
!               taua(l,11) = tauaer_MacV2(iplon, 9,nlay+1-l)
!               taua(l,12) = tauaer_MacV2(iplon,10,nlay+1-l)
!               taua(l,13) = tauaer_MacV2(iplon,10,nlay+1-l)
!               taua(l,14) = tauaer_MacV2(iplon,11,nlay+1-l)                 
!               taua(l,15) = tauaer_MacV2(iplon,11,nlay+1-l)
!               taua(l,16) = tauaer_MacV2(iplon,12,nlay+1-l)
!             else
              taua(l,1) = tauaer(iplon,1,nlay+1-l)
              taua(l,2) = tauaer(iplon,1,nlay+1-l)
              taua(l,3) = tauaer(iplon,2,nlay+1-l)                        
              taua(l,4) = tauaer(iplon,2,nlay+1-l)
              taua(l,5) = tauaer(iplon,2,nlay+1-l)
              taua(l,6) = tauaer(iplon,3,nlay+1-l)
              taua(l,8) = tauaer(iplon,3,nlay+1-l)
              taua(l,9) = tauaer(iplon,3,nlay+1-l)  
              taua(l,7) = tauaer(iplon,4,nlay+1-l)
              taua(l,10) = tauaer(iplon,5,nlay+1-l)
              taua(l,11) = tauaer(iplon,5,nlay+1-l)
              taua(l,12) = tauaer(iplon,5,nlay+1-l)
              taua(l,13) = tauaer(iplon,5,nlay+1-l)
              taua(l,14) = tauaer(iplon,5,nlay+1-l)                 
              taua(l,15) = tauaer(iplon,5,nlay+1-l)
              taua(l,16) = tauaer(iplon,5,nlay+1-l)
           ! end if
         enddo
      endif
