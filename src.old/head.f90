      ...
      Program rrtmglw_test
        ...
        Integer(Kind=4), Parameter :: KEND = 2 !401
          ...
          if (.True.) then
            do k = 1,KLEV+1
              write(*,'(i8,4(e15.5),a,4(e15.5),a,100(e15.5))') k,\
                  UFLX (k),UFLX_ref (k),UFLX (k)-UFLX_ref (k),(UFLX (k)-UFLX_ref (k))/max(1.E-10,UFLX_ref (k)), \
                  ' - ',\
                  DFLX (k),DFLX_ref (k),DFLX (k)-DFLX_ref (k),(DFLX (k)-DFLX_ref (k))/max(1.E-10,DFLX_ref (k)), \
                  ' - ',\
